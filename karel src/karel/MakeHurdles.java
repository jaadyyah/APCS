...
public class MakeHurdles
{
	...
	// example: northsouthwalls 2 1 5 draws vert wall between ave 2 and 3 crossing st 1 - 5
	private static int buildHurdle(int currentAve, int width, int height)
	{
		...
		if (width > 0)
		{
			...
			// Occasionally place a beeper on the hurdle
			if (Math.random() < 0.4) // 4% of the time
			{
				placeBeeper(height + 1, (int)(Math.random() * width) + currentAve + 1, 1);
			}
		}
