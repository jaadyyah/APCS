...
public class TextExcel
{
	...
	private static void handleCommand(String command, Spreadsheet sheet)
	{
		...
		if (command.startsWith("sorta"))
		{
			sheet.sortAscending(command);
			return;
		}
		if (command.startsWith("sortd"))
		{
			sheet.sortDescending(command);
			return;
		}
		if (command.length() > 7)
		{
			if (command.substring(0, 5).equals("clear") && sheet.isCellReference(command.substring(6)))
			{
				sheet.clear(command.substring(6));
				return;
			}
		}
